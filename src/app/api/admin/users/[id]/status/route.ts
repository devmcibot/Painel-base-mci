...
export async function PUT(req: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    // ATENÇÃO: aqui usamos o campo do Prisma -> hashedPwd
    ...
    const updatedUser = await prisma.$transaction(async (tx) => {
      ...
      // Se for MÉDICO, garante/atualiza o registro em Medico com o CRM
      if (role === "MEDICO") {
        if (existing.medico) {
        ...
        // Se virar ADMIN, só zera CRM se existir Medico
        if (existing.medico) {
          await tx.medico.update({
            where: { id: existing.medico.id },
            data: { crm: null },
...
export async function DELETE(req: NextRequest, { params }: RouteContext) {
  ...
  try {
    // Verifica se tem médico vinculado e se ele possui vínculos
